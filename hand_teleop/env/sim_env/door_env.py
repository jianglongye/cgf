...
class DoorEnvHeuristics:
    ...
    def solve(self):
        ...
        if self.stage == 0:
            pre_grasp_pos = (
                center + distance_sign * np.array([0.20, 0.02, 0]) + np.array([0, 0, -0.01]) - self.robot.get_pose().p
            )
    ...
    def step(self):
        ...
        if self.stage == 0:
            ...
            qpos[4] = board_angle
            qpos[0] = (
                self.current_qpos_target[0] - door_length * np.sin(board_angle) - np.sin(board_angle) * palm_length
            )
            qpos[1] = (
                self.current_qpos_target[1]
                + door_length * (np.cos(board_angle) - 1)
                - np.sin(board_angle) * palm_length
            )
