...
class BaseSimulationEnv(object):
    def __init__(self, use_gui=True, frame_skip=5, use_visual_obs=False, no_rgb=False, **renderer_kwargs):
        engine, renderer = get_engine_and_renderer(
            use_gui=use_gui, use_visual_obs=use_visual_obs, no_rgb=no_rgb, **renderer_kwargs
        )
    ...
    def create_camera(
        ...
        if mount_actor_name is not None:
            ...
            cam = self.scene.add_mounted_camera(
                name, mount, Pose(), width=resolution[0], height=resolution[1], fovy=fov, fovx=fov, near=0.1, far=10
            )
        else:
    ...
    def create_camera_from_pose(
        self, pose: sapien.Pose, name: str, resolution: Sequence[Union[float, int]], fov: float
    ):
        if not len(resolution) == 2:
            raise ValueError(f"Resolution should be a 2d array, but now {len(resolution)} is given.")
