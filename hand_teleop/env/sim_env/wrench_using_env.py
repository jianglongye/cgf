...
class WrenchUsingEnv(BaseSimulationEnv):
    ...
    def load_hex_bolt_and_driver(self):
        ...
        viz_mat_bolt = create_visual_material(
            self.renderer, 0.1, 0.5, 0.4, base_color=np.array([80, 72, 74, 255]) / 255
        )
        physical_mat = self.scene.create_physical_material(1.5, 1, 0)

        # Build hex bolt
        root_link.add_box_visual(
            pose=sapien.Pose([0, 0, bottom_half_size[2]]), half_size=bottom_half_size, material=viz_mat_bolt
        )
        root_link.add_box_collision(
            pose=sapien.Pose([0, 0, bottom_half_size[2]]), half_size=bottom_half_size, material=physical_mat
        )
        ...
        child_link.set_joint_properties(
            "revolute",
        ...
        # Build hex driver
        density = 400
        viz_mat = create_visual_material(self.renderer, 0.2, 0.8, 0.4, base_color=np.array([140, 138, 132, 255]) / 255)
        ...
        pos_list = [
            (extend, 0, half_driver_height),
            (-extend, 0, half_driver_height),
            (0, extend, half_driver_height),
            (0, -extend, half_driver_height),
        ]
        half_size_list = [
        ...
        for i in range(4):
            ...
            builder.add_box_collision(
                sapien.Pose(np.array(pos_list[i])), half_size_list[i], material=physical_mat, density=density * 2
            )
        ...
        builder.add_capsule_visual(
            sapien.Pose([half_width + radius + handle_length / 2, 0, handle_height]),
            radius=radius,
            half_length=handle_length / 2,
            material=viz_mat,
        )
