...
class TableDoorEnv(BaseSimulationEnv):
    ...
    def load_door(self):
        ...
        root.add_capsule_collision(
            pose=sapien.Pose([0, 0.185, 0.2], transforms3d.euler.euler2quat(0, -np.pi / 2, 0)),
            radius=0.025,
            half_length=0.175,
        )
        root.add_capsule_collision(
        ...
        door.add_box_collision(
            pose=sapien.Pose([0, 0, 0.2]), half_size=[0.025, 0.15, 0.2], density=100, material=table_physics_mat
        )
        ...
        hinge.add_capsule_collision(
            pose=sapien.Pose([-0.02, 0.10, 0.2]), radius=0.015, half_length=0.07, material=table_physics_mat
        )
        hinge.add_capsule_collision(
            pose=sapien.Pose([-0.09, 0.05, 0.2], [0.707, 0, 0, 0.707]),
            radius=0.015,
            half_length=0.05,
            material=table_physics_mat,
        )
        hinge.add_capsule_collision(
            pose=sapien.Pose([0.05, 0.14, 0.2], [0.707, 0, 0, 0.707]),
            radius=0.015,
            half_length=0.04,
            material=table_physics_mat,
        )
        ...
        if self.use_gui:
            ...
            root.add_capsule_visual(
                pose=sapien.Pose([0, 0.185, 0.2], transforms3d.euler.euler2quat(0, -np.pi / 2, 0)),
                radius=0.025,
                half_length=0.175,
                material=frame_viz_mat,
            )
            root.add_capsule_visual(
                pose=sapien.Pose([0, -0.185, 0.2], transforms3d.euler.euler2quat(0, -np.pi / 2, 0)),
                radius=0.025,
            ...
            hinge.add_capsule_visual(
                pose=sapien.Pose([-0.09, 0.05, 0.2], [0.707, 0, 0, 0.707]), radius=0.015, half_length=0.05
            )
            hinge.add_capsule_visual(
                pose=sapien.Pose([0.05, 0.14, 0.2], [0.707, 0, 0, 0.707]), radius=0.015, half_length=0.04
            )
