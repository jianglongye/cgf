...
class FetchWaterEnv(BaseSimulationEnv):
    def __init__(self, use_gui=True, frame_skip=5, coffee_machine_id=1000, **renderer_kwargs):
        ...
        self.coffee_visual_material = create_visual_material(
            self.renderer, 0.5, 0, 0.4, np.array([212, 241, 249, 100]) / 255
        )
        coffee_output_link_name = "link_5"
        self.coffee_output_link = [
            link for link in self.coffee_machine.get_links() if link.get_name() == coffee_output_link_name
        ][0]
