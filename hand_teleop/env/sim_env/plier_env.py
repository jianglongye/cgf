...
class PlierEnv(BaseSimulationEnv):
    ...
    def load_tower_and_block(self):
        ...
        tower = create_tower(
            self.scene,
            self.renderer,
            inner_size=tower_inner_half_size - 0.002,
            shell_size=shell_size,
            shell_thickness=shell_thickness / 2,
            shell_offset=shell_offset,
            base_thickness=base_thickness / 2,
            pin_height=pin_height,
        )

        block, support = create_hole_block_and_support(
            self.scene, self.renderer, block_outer_half_size, tower_inner_half_size, block_thickness / 2
        )
        return tower, block, support


def create_tower(
    scene: sapien.Scene, renderer, inner_size, shell_size, shell_thickness, shell_offset, base_thickness, pin_height
):
    builder = scene.create_actor_builder()
    viz_mat = create_visual_material(renderer, 0.1, 0.5, 0.4, base_color=np.array([80, 72, 74, 255]) / 255)
    pin_viz_mat = create_visual_material(renderer, 0.1, 0.1, 0.4, base_color=np.array([180, 172, 174, 255]) / 255)
    # Build base
...
def create_hole_block_and_support(scene: sapien.Scene, renderer, outer_size, inner_size, thickness):
    ...
    viz_mat = create_visual_material(renderer, 0.1, 0.5, 0.4, base_color=np.array([40, 36, 37, 255]) / 255)
    for i in range(4):
        builder.add_box_visual(
            sapien.Pose(np.array([block_xy[i][0], block_xy[i][1], thickness / 2])),
    ...
    builder = scene.create_actor_builder()
    builder.add_box_visual(
        sapien.Pose(np.array([0, 0, thickness])), np.array([R / 2, R / 2, thickness]), color=np.array([0, 0, 0.5, 1])
    )
