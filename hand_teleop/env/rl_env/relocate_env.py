...
class LabArmAllegroRelocateRLEnv(LabRelocateEnv, BaseRLEnv):
    def __init__(
        ...
        **renderer_kwargs,
        ...
        super().__init__(
            use_gui, frame_skip, object_category, object_name, randomness_scale, friction, **renderer_kwargs
        )
        ...
        self.finger_tip_names = ["link_15.0_tip", "link_3.0_tip", "link_7.0_tip", "link_11.0_tip"]
        robot_link_names = [link.get_name() for link in self.robot.get_links()]
        self.finger_tip_links = [self.robot.get_links()[robot_link_names.index(name)] for name in self.finger_tip_names]
        self.finger_tip_pos = np.zeros([len(self.finger_tip_names), 3])
        ...
        self.object_in_tip = np.zeros([len(self.finger_tip_names), 3])
        self.target_in_object = np.zeros([3])
        self.target_in_object_angle = np.zeros([1])

        # Contact buffer
        self.robot_object_contact = np.zeros(len(self.finger_tip_names) + 1)  # four tip, palm
    ...
    def update_cached_state(self):
        ...
        check_contact_links = self.finger_tip_links + [self.palm_link]
        self.robot_object_contact[:] = self.check_actor_pair_contacts(check_contact_links, self.manipulated_object)
        self.object_pose = self.manipulated_object.get_pose()
        self.palm_pose = self.palm_link.get_pose()
    ...
    def get_robot_state(self):
        robot_qpos_vec = self.robot.get_qpos()
        return np.concatenate(
            [robot_qpos_vec, self.palm_pos_in_base, self.target_pose.p - self.base_frame_pos, self.target_pose.q]
        )
    ...
    def get_reward(self, action):
        ...
        is_contact = np.sum(self.robot_object_contact) >= 2  # at least one tip and palm or two tips are contacting obj

        if is_contact:
            reward += 0.1
            lift = np.clip(self.object_pose.p[2] - self.object_height, 0, 0.2)
            reward += 10 * lift
            if lift > 0.02:
                reward += 1
                ...
                if target_obj_dist < 0.1:
                    ...
                    reward += 4.0 / (0.2 + theta) * self.rotation_reward_weight
