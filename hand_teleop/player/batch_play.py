...
from kinematics.retargeting_optimizer import PositionRetargeting
...
def batch_play_data(task_name, robot_name, dir_path, num_cpu, version_num):
    ...
    if num_cpu == 1:
        ...
        results = [
            pool.apply(play_one_data, args=(task_name, robot_name, all_path_list[i])) for i in range(len(all_path_list))
        ]
...
def play_one_data(task_name, robot_name, filename):
    ...
    if use_same_hand:
        ...
        env.robot.set_pose(sapien.Pose(np.array([-0.2, 0, 0.2]), transforms3d.euler.euler2quat(0, np.pi / 2, 0)))
