...
class DataPlayer:
    ...
    def get_sim_data(self, item) -> Dict[str, Any]:
        ...
        scene_actor_data = {
            self.meta2scene_actor[key]: value for key, value in actor_data.items() if key in self.meta2scene_actor
        }
        scene_drive_data = {
    ...
    def get_finger_tip_retargeting_result(
        self, human_robot_hand: MANORobotHand, retargeting: PositionRetargeting, indices, use_root_local_pose=True
    ):
            ...
            if use_human_hand:
                if method == "tip_middle":
                    qpos = self.get_finger_tip_middle_retargeting_result(
                        self.human_robot_hand, retargeting, indices, use_root_local_pose=use_local_pose
                    )
                elif method == "tip":
            ...
            if i >= 1:
                baked_data["action"].append(
                    self.compute_action_from_states(
                        baked_data["robot_qpos"][i - 1], qpos, np.sum(contact_finger_index) > 0
                    )
                )
            if i >= 2:
...
class TableDoorEnvPlayer(DataPlayer):
    ...
    def bake_demonstration(self, retargeting: Optional[PositionRetargeting] = None, method="tip_middle", indices=None):
        ...
        for i in range(self.meta_data["data_len"]):
            ...
            # Environment state
            baked_data["state"].append(self.collect_env_state(articulations=[table_door]))
...
def bake_demonstration_allegro_test():
    ...
    retargeting = PositionRetargeting(
        env.robot, joint_names, link_names, has_global_pose_limits=False, has_joint_limits=True
    )
    ...
    method = "dapg"
    output_dir = Path(__file__).parent / "real_trajectory" / "relocate" / env.object_name / method
    output_dir.mkdir(exist_ok=True, parents=True)
    qpos_list = []
    init_pos = baked_data["state"][0]["actor"][actor_id]["pose"][:2]
    target_pos = baked_data["state"][-1]["actor"][actor_id]["pose"][:2]
    filename = (
        f"{int(init_pos[0] * 100)}_{int(init_pos[1] * 100)}_{int(target_pos[0] * 100)}_{int(target_pos[1] * 100)}.npy"
    )

    env.render()
    env.viewer.toggle_pause(True)
    for obs, qpos, state, action in zip(
        baked_data["obs"], baked_data["robot_qpos"], baked_data["state"], baked_data["action"]
    ):
