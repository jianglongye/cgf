...
def generate_free_robot_hand_info() -> Dict[str, FreeRobotInfo]:
    shadow_hand_free_info = FreeRobotInfo(
        path="robot/shadow_hand_description/shadowhand_free.urdf", dof=28, palm_name="palm_center"
    )
    ...
    allegro_hand_free_info = FreeRobotInfo(
        path="robot/allegro_hand_description/allegro_hand_free.urdf", dof=22, palm_name="palm_center"
    )
...
def load_robot(scene: sapien.Scene, robot_name) -> sapien.Articulation:
    ...
    for link_builder in robot_builder.get_link_builders():
        link_builder.set_collision_groups(0, 1, 2, 0)
