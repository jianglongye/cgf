...
from hand_teleop.utils.model_utils import (
    build_free_root,
    rot_from_connected_link,
    build_ball_joint,
    fix_link_inertia,
...
def create_mano_articulated_robot_hand(
    scene: sapien.Scene,
    renderer: sapien.VulkanRenderer,
    ...
    for i in range(4):
        ...
        pos = np.array([-finger_palm_length / 2, 0, joint_pos[5, LITTLE_TO_THUMB_AXIS] - i * finger_palm_width])
        palm.add_box_visual(
            pose=Pose(pos),
    ...
    for i in range(5):
        ...
        rot_01 = rot_from_connected_link(joint_pos[1 + 4 * i], joint_pos[2 + 4 * i])
        quat_01 = mat2quat(rot_01)

        # First finger link
        quat_y_rotate = euler2quat(0, 0, np.pi / 2)
        finger_00.set_joint_name(f"{finger_names[i]}_00_joint")
        finger_00.set_joint_properties(
            "revolute",
            limits=np.array([limit_finger_00[i]]),
            pose_in_parent=Pose(joint_pos[1 + 4 * i], qmult(quat_01, quat_y_rotate)),
        ...
        finger_01.set_joint_properties(
            "revolute",
        ...
        finger_01.add_capsule_visual(
            pose=Pose([-height_01 / 2, 0, 0]), radius=FINGER_RADIUS, half_length=height_01 / 2, material=finger_viz_mat
        )

        # Second finger link
        rot_11 = rot_from_connected_link(joint_pos[2 + 4 * i], joint_pos[3 + 4 * i], parent_rotation=rot_01)
        ...
        finger_11.set_joint_properties(
            "revolute",
            limits=np.array([limit_finger_11[i]]),
            pose_in_parent=Pose([-height_01, 0, 0], q=qmult(quat_11, rot_x_to_z)),
            pose_in_child=Pose(q=rot_x_to_z),
        )
        finger_11.add_capsule_visual(
            pose=Pose([-height_11 / 2, 0, 0]), radius=FINGER_RADIUS, half_length=height_11 / 2, material=finger_viz_mat
        )

        # Third finger link
        rot_21 = rot_from_connected_link(joint_pos[3 + 4 * i], joint_pos[4 + 4 * i], parent_rotation=rot_01 @ rot_11)
        ...
        finger_21.set_joint_properties(
            "revolute",
            limits=np.array([limit_finger_21[i]]),
            pose_in_parent=Pose([-height_11, 0, 0], q=qmult(quat_21, rot_x_to_z)),
            pose_in_child=Pose(q=rot_x_to_z),
        )
        finger_21.add_capsule_visual(
            pose=Pose([-height_21 / 2, 0, 0]), radius=FINGER_RADIUS, half_length=height_21 / 2, material=tip_viz_mat
        )
        ...
        finger_31.set_joint_properties(
            "fixed", limits=np.array([]), pose_in_parent=Pose([-height_31, 0, 0]), pose_in_child=Pose()
        )
...
def create_mano_full_dof_robot_hand(
    scene: sapien.Scene,
    renderer: sapien.VulkanRenderer,
    joint_pos: np.ndarray,
    ...
    for i in range(5):
        ...
        if use_visual:
            link0.add_capsule_visual(
                pose=Pose(pos_link1_in_link0 / 2, quat_link1_in_link0),
                radius=radius,
                half_length=length_link1_in_link0 / 2,
                material=finger_viz_mat,
            )
        link0.add_capsule_collision(
            pose=Pose(pos_link1_in_link0 / 2, quat_link1_in_link0),
            radius=radius,
            half_length=length_link1_in_link0 / 2,
            **friction_dict,
        ...
        if use_visual:
            link1.add_capsule_visual(
                pose=Pose(pos_link2_in_link1 / 2, quat_link2_in_link1),
                radius=radius,
                half_length=length_link2_in_link1 / 2,
                material=finger_viz_mat,
            )
        link1.add_capsule_collision(
            pose=Pose(pos_link2_in_link1 / 2, quat_link2_in_link1),
            radius=radius,
            half_length=length_link2_in_link1 / 2,
            **friction_dict,
        ...
        if use_visual:
            link2.add_capsule_visual(
                pose=Pose(pos_link3_in_link2 / 2, quat_link3_in_link2),
                radius=radius,
                half_length=length_link3_in_link2 / 2,
                material=tip_viz_mat,
            )
        link2.add_capsule_collision(
            pose=Pose(pos_link3_in_link2 / 2, quat_link3_in_link2),
            radius=radius,
            half_length=length_link3_in_link2 / 2,
            **friction_dict,
        ...
        link3.set_joint_properties(
            "fixed", limits=np.array([]), pose_in_parent=Pose(joint_pos[4 + 4 * i] - joint_pos[3 + 4 * i])
        )
...
def _create_palm(robot_builder: sapien.ArticulationBuilder, free_root: bool, robot_name=""):
    if free_root:
        ...
        palm.set_joint_properties(
            "fixed", limits=np.array([]), pose_in_parent=Pose(q=euler2quat(0, np.pi / 2, np.pi / 2))
        )
    palm.set_name("palm")
    return palm
...
def main():
    ...
    smplx_model_path = Path(__file__).parent.parent / "hand_detector" / "extra_data" / "smpl" / "SMPLX_NEUTRAL.pkl"
    smplx = smplx.create(
        str(smplx_model_path.resolve()),
        model_type="smplx",
        batch_size=1,
        gender="neutral",
        num_betas=10,
    ...
    shape_params = (
        np.array(
            [[-0.6953, -0.0172, 0.1187, -0.0556, -0.0531, -0.0848, 0.0136, -0.0182, -0.0680, -0.0583]], dtype=np.float32
        )
        + np.random.randn(1, 10) * 0.3
    )
    ...
    def forward_smplx(pose_param):
        with torch.no_grad():
            output = smplx(body_pose=body_pose, right_hand_pose=pose_param, betas=shape_params, return_verts=True)
    ...
    render_scene.add_directional_light(np.array([1, -1, -1]), np.array([1, 1, 1]))
    ...
    mat_hand = context.create_material(np.array([0.96, 0.75, 0.69, 1]), 0.0, 0.8, 0)
    mano_robot = MANORobotHand(scene, renderer, joint_pos, full_dof=True, free_root=True, control_interval=0.1)
    ...
    while not viewer.closed:
        ...
        hand_qpos = mano_robot.compute_qpos((hand_pose + smplx.right_hand_mean).cpu().numpy().flatten())
        hand_verts, joint_pos = forward_smplx(hand_pose)
        # Rendering
        obj = add_mesh_to_renderer(scene, renderer, hand_verts, faces, material=mat_hand)
        line_set = add_line_set_to_renderer(
            scene, renderer, joint_pos, np.stack([parent, np.arange(21)], axis=1), color=np.array([1, 0, 0, 1])
        )
        line = add_line_set_to_renderer(
            scene, renderer, joint_pos, np.stack([parent, np.arange(21)], axis=1), color=np.array([1, 0, 0, 1])
        )
        ...
        print(f"Shape param: {(hand_pose + smplx.right_hand_mean).cpu().numpy()[:3]}")
