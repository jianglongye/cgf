...
class PartialKinematicModel:
    def __init__(self, robot: sapien.Articulation, start_joint_name: str, end_joint_name: str):
        ...
        self.start_joint_tuple = [
            (joint, num) for num, joint in enumerate(robot.get_joints()) if joint.get_name() == start_joint_name
        ][0]
        ...
        for j_idx, j in enumerate(all_joints):
            ...
            link.set_joint_properties(j.type, j.get_limits(), j.get_pose_in_parent(), j.get_pose_in_child())
        ...
        self.end_link_index = [
            i for i, link in enumerate(partial_robot.get_links()) if link.get_name() == self.end_link_name
        ][0]
